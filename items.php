...
class Items extends productsDB{
    ...
    public function getSearchItems($text){
        return $this->query('select * from items where name = "'.$text.'" or description = "'.$text.'" and category in (select name from categories where status = "show")');
    }

    public function getShowItems(){
        // select * from items where category in (select name from categories where status = show)
        return $this->query('select * from items where category in (select name from categories where status = "show")');
    }
